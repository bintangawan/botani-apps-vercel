<x-manage-layout title="Tambah Modul Pembelajaran Baru — Botani Phanerogamae">
    <div class="space-y-8 max-w-4xl mx-auto">
        ...
        <div class="bg-gradient-to-r from-[#0c2214] via-emerald-950 to-[#07130c] border border-emerald-500/30 rounded-3xl p-6 sm:p-8 shadow-xl flex items-center justify-between gap-6">
            <div class="space-y-1.5">
                <span class="px-3 py-1 rounded-full bg-teal-500/20 text-teal-300 text-xs font-bold border border-teal-500/30">📚 Formulir Modul</span>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-white">Tambah Modul Pembelajaran</h1>
                <p class="text-sm text-slate-300">
                    Susun materi kuliah, teori ilmiah taksonomi, serta hubungkan spesimen tanaman terkait.
                </p>
            </div>
        ...
        <!-- Create Form -->
        <form action="{{ route('manage.modules.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="bg-[#0c2214]/60 border border-emerald-900/50 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Contoh: Modul 1: Pengantar Botani Phanerogamae" 
                    ...
                    <div>
                        ...
                        <input type="number" name="module_order" id="module_order" value="{{ old('module_order', 1) }}" required min="1" 
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="2" placeholder="Ringkasan atau kompetensi dasar yang akan dicapai mahasiswa pada modul ini..." 
                ...
                <div>
                    ...
                    <div id="quill-editor" class="text-white">{!! old('content') !!}</div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 max-h-64 overflow-y-auto p-4 rounded-2xl bg-[#07130c] border border-emerald-900/60 gap-3">
                        @forelse($plants as $pl)
                            <label class="flex items-center gap-3 p-2 rounded-xl hover:bg-emerald-950/50 cursor-pointer transition-colors border border-transparent hover:border-emerald-800/40">
                                <input type="checkbox" name="species[]" value="{{ $pl->id }}" class="rounded bg-emerald-950 border-emerald-700 text-emerald-500 focus:ring-emerald-500 w-4 h-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2 border-t border-emerald-900/50">
                    <div>
                        ...
                        <select name="status" id="status" required class="w-full rounded-xl bg-[#07130c] border border-emerald-900/60 focus:border-emerald-500 text-white text-sm py-2.5 px-4 font-bold">
                            <option value="published" selected>🟢 Dipublikasi (Dapat dipelajari Mahasiswa)</option>
                            <option value="draft">🟡 Draf (Menunggu Penyempurnaan)</option>
                        </select>
                    </div>
                </div>
            </div>
