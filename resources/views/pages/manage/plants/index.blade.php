    ...
    <div class="space-y-8">
        ...
        <div class="bg-gradient-to-r from-[#0c2214] via-emerald-950 to-[#07130c] border border-emerald-500/30 rounded-3xl p-6 sm:p-8 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6">
            <div class="space-y-1.5 text-center sm:text-left">
                <span class="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-500/30">🌱 Katalog & Taksonomi</span>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-white">Manajemen Spesimen Tumbuhan</h1>
            ...
            <a href="{{ route('manage.plants.create') }}" class="px-6 py-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white font-extrabold text-sm shadow-lg shadow-emerald-600/30 flex items-center gap-2.5 shrink-0 transition-all">
        ...
        <div class="bg-[#0c2214]/60 border border-emerald-900/50 rounded-2xl p-4 sm:p-6 shadow-lg">
            <form action="{{ route('manage.plants.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div class="md:col-span-2 space-y-1.5">
                    <label for="search" class="block text-xs font-bold uppercase tracking-wider text-emerald-400">Pencarian Spesimen</label>
                ...
                <div class="space-y-1.5">
                    <label for="group_type" class="block text-xs font-bold uppercase tracking-wider text-emerald-400">Kelompok Tumbuhan</label>
                    <select name="group_type" id="group_type" class="w-full rounded-xl bg-[#07130c] border border-emerald-900/60 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-white text-sm py-2.5 px-4">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->hasAny(['search', 'group_type']))
                        <a href="{{ route('manage.plants.index') }}" class="py-2.5 px-4 rounded-xl bg-[#07130c] border border-emerald-900/60 hover:bg-emerald-900/40 text-slate-300 font-bold text-sm transition-all text-center">
                            Reset
                        </a>
                    @endif
                </div>
                    ...
                    <tbody class="divide-y divide-emerald-950/60 text-slate-300">
                        @forelse($plants as $pl)
                            <tr class="hover:bg-emerald-950/40 transition-colors">
                                <td class="py-4 px-4 font-bold text-white flex items-center gap-3.5">
                                    <div class="w-12 h-12 rounded-xl bg-[#07130c] border border-emerald-900/60 overflow-hidden shrink-0 flex items-center justify-center">
                                        @if($pl->image_url)
                                    ...
                                    <div>
                                        <span class="font-mono text-xs text-emerald-400 block">{{ $pl->code }}</span>
                                        <span class="text-[10px] text-slate-500 uppercase font-mono tracking-widest">{{ $pl->cotyledon_type ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="py-4 px-4 font-bold text-white">{{ $pl->local_name }}</td>
                                <td class="py-4 px-4 font-serif italic text-emerald-300 text-base">
                                    {{ $pl->scientific_name }}
                                    @if($pl->author_name)
                                        <span class="text-xs font-sans not-italic text-slate-400">{{ $pl->author_name }}</span>
                                    @endif
                                </td>
                                <td class="py-4 px-4">
                                    <span class="px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider {{ $pl->group_type === 'Gymnospermae' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' }}">
                                        {{ $pl->group_type }}
                                    </span>
                                </td>
                                <td class="py-4 px-4">
                                    <span class="px-2.5 py-1 rounded text-xs font-bold {{ $pl->status === 'published' ? 'text-emerald-400 bg-emerald-950/60' : 'text-amber-400 bg-amber-950/60' }}">
