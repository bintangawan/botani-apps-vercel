    ...
    <div class="space-y-8">
        ...
        <div class="bg-emerald-100 border border-emerald-300 rounded-3xl p-6 sm:p-8 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6">
            <div class="space-y-1.5 text-center sm:text-left">
                <span class="px-3 py-1 rounded-full bg-amber-100 text-amber-700 text-xs font-bold border border-amber-300">🎯 Bank Soal & Kuis</span>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900">Manajemen Kuis & Evaluasi</h1>
                <p class="text-sm text-slate-700">
                    Kelola konfigurasi ujian kuis, nilai ketuntasan minimal (passing score), serta susun bank soal dan kunci jawaban.
                </p>
            </div>
            <a href="{{ route('manage.quizzes.create') }}" class="px-6 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-slate-900 font-extrabold text-sm shadow-lg shadow-emerald-900/10 flex items-center gap-2.5 shrink-0 transition-all">
        ...
        <div class="bg-white border border-emerald-200 rounded-2xl p-4 sm:p-6 shadow-lg">
            <form action="{{ route('manage.quizzes.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div class="md:col-span-2 space-y-1.5">
                    <label for="search" class="block text-xs font-bold uppercase tracking-wider text-emerald-700">Pencarian Kuis</label>
                ...
                <div class="space-y-1.5">
                    <label for="module_id" class="block text-xs font-bold uppercase tracking-wider text-emerald-700">Filter Modul Perkuliahan</label>
                    <select name="module_id" id="module_id" class="w-full rounded-xl bg-white border border-emerald-200 focus:border-emerald-500 text-slate-900 text-sm py-2.5 px-4">
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->hasAny(['search', 'module_id']))
                        <a href="{{ route('manage.quizzes.index') }}" class="py-2.5 px-4 rounded-xl bg-white border border-emerald-200 hover:bg-emerald-100 text-slate-700 font-bold text-sm transition-all text-center">
                            Reset
                        </a>
                    @endif
                </div>
                    ...
                    <tbody class="divide-y divide-emerald-100 text-slate-700">
                        @forelse($quizzes as $qz)
                            <tr class="hover:bg-emerald-100 transition-colors">
                                <td class="py-4 px-4">
                                    <span class="font-extrabold text-slate-900 text-base block mb-0.5">{{ $qz->title }}</span>
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-700 border border-emerald-300">
                                ...
                                <td class="py-4 px-4 text-center">
                                    <span class="px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 font-extrabold text-xs border border-emerald-300">
                                ...
                                <td class="py-4 px-4 text-center">
                                    <a href="{{ route('manage.quizzes.questions.index', $qz) }}" 
                                       class="px-3.5 py-1.5 rounded-xl bg-amber-100 hover:bg-amber-200 text-amber-700 border border-amber-300 transition-all font-bold text-xs inline-flex items-center gap-1.5 shadow-sm">
                                        <span>📝 Kelola Soal ({{ $qz->questions_count ?? 0 }})</span>
                                    </a>
                                </td>
                                <td class="py-4 px-4 text-center">
                                    <span class="px-2.5 py-1 rounded text-xs font-bold {{ $qz->status === 'published' ? 'text-emerald-700 bg-emerald-50' : 'text-amber-700 bg-amber-50' }}">
